...
const PDFViewerApplication = {
  ...
  async run(config) {
    ...
    if (params.get("file")) {
      ...
      file = params.get("view") ?? _app_options_js__WEBPACK_IMPORTED_MODULE_9__.AppOptions.get("defaultUrl");
    }
  ...
  bindEvents() {
    ...
    eventBus._on("openfileinnewtab", webViewerOpenInNewTab);
    ...
    document.getElementById("viewerContainer").addEventListener("dblclick", function (e) {
      PDFViewerApplication.pdfViewer.currentScaleValue = PDFViewerApplication.pdfViewer.currentScaleValue === "page-width" ? "page-height" : "page-width";
    });
  },
  bindWindowEvents() {
    const {
...
{
  const HOSTED_VIEWER_ORIGINS = ["null", "http://mozilla.github.io", "https://mozilla.github.io", "http://pdfjs-20240412.creativecirclemedia.com", "https://pdfjs-20240412.creativecirclemedia.com", "http://pdfjs.creativecirclemedia.com", "https://pdfjs.creativecirclemedia.com"];
...
function webViewerOpenInNewTab() {
  window.open(window.currentFile);
}
...
function webViewerKeyDown(evt) {
  ...
  let tmpVar = 1;
  if (evt.keyCode !== 61 && evt.keyCode !== 107 && evt.keyCode !== 187 && evt.keyCode !== 171 && evt.keyCode !== 173 && evt.keyCode !== 109 && evt.keyCode !== 189) {
    tmpVar = (evt.ctrlKey ? 1 : 0) | (evt.altKey ? 2 : 0) | (evt.shiftKey ? 4 : 0) | (evt.metaKey ? 8 : 0);
  }
  const cmd = tmpVar;
...
function unsupportedIosPresent() {
  if (/(DuckDuckGo|Opera)/.test(navigator.userAgent)) {
    return true;
  } else if (/iP(hone|od|ad)/.test(navigator.platform)) {
    const appVersion = navigator.appVersion.match(/OS (\d+)_(\d+)_?(\d+)?/);
    const iosVersion = [parseInt(appVersion[1], 10), parseInt(appVersion[2], 10), parseInt(appVersion[3] || 0, 10)];
    if (iosVersion[0] === 16 && iosVersion[1] === 3) {
      return true;
    }
  }
  return false;
...
class Toolbar {
  ...
  constructor(options, eventBus) {
    ...
    const buttons = [{
      ...
      element: options.openFileInNewTab,
      eventName: "openfileinnewtab",
      close: true
    }];
    this.#bindListeners(buttons);
...
const pdfjsVersion = "4.1.436";
const pdfjsBuild = "968e6b60b";
const AppConstants = {
...
function getViewerConfiguration() {
  return {
    ...
    toolbar: {
      ...
      openFileInNewTab: document.getElementById("openFileInNewTab"),
